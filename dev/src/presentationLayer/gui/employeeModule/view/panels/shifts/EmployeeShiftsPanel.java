...
public class EmployeeShiftsPanel extends AbstractTransportModulePanel {
    EmployeeView employeeView;
    JPanel newOpenPanel = new JPanel(), myShiftsPanel = new JPanel();
    JFrame newOpenWindow, myShiftsWindow;
    ...
    public EmployeeShiftsPanel(String employeeId, ShiftsControl control, EmployeeView employeeView) {
        super(control);
    ...
    private void init() {
        ...
        // Initialize "My Shifts" panel

        PrettyButton myShiftsButton = new PrettyButton("My Shifts");
        myShiftsButton.setPreferredSize(new Dimension(200, 30));
        myShiftsButton.setBounds(350, 550, 200, 30);

        myShiftsButton.addActionListener(new ActionListener() {
    ...
    private void openMyShiftsWindow() {
        myShiftsWindow = new JFrame();
        myShiftsWindow.setSize(900, 650);
        myShiftsWindow.setLocationRelativeTo(contentPanel);
        myShiftsWindow.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

        myShiftsWindow.getContentPane();
        myShiftsPanel.setLayout(null);
        myShiftsWindow.add(myShiftsPanel);

        JLabel myShiftsLabel = new JLabel("My Shifts:");
        myShiftsPanel.add(myShiftsLabel);
        Object result = ((ShiftsControl)control).getEmployeeShifts(employeeId);
        if (result instanceof String) {
            JOptionPane.showMessageDialog(myShiftsPanel, result);
        } else if (result instanceof List) {
            List<SShift[]> myShifts = (List<SShift[]>) result;
            for (SShift[] shifts : myShifts) {
                ...
                if (shifts[0] != null) {
                    ...
                    myShiftsPanel.add(morningShiftScrollPane);
                }
                if (shifts[1] != null) {
                    ...
                    myShiftsPanel.add(eveningShiftScrollPane);
                }
            }
            if (myShifts.size() == 0) {
            ...
            myShiftsPanel.repaint();
            myShiftsWindow.revalidate();
            myShiftsWindow.repaint();
            myShiftsWindow.addWindowListener(new WindowAdapter() {
                @Override
                public void windowClosing(WindowEvent windowEvent) {
                    int month = calendar.get(Calendar.MONTH);
                    employeeView.setCurrentPanel(new CancelCardPanel(employeeId, (ShiftsControl)control, employeeView));
                    employeeView.setCurrentPanel(new EmployeeShiftsPanel(employeeId, (ShiftsControl) control, employeeView));
                    calendar.set(Calendar.MONTH, month);
                }
            });
            myShiftsWindow.setVisible(true);
        } else {
            JOptionPane.showMessageDialog(myShiftsPanel, "An unexpected result as returned from the employees service.");
        }
    }
